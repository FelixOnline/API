...
function email_article_comment($article,$user,$comment,$commentid,$name=NULL) {
    ...
    $body .= '"'.stripslashes(str_replace('\r\n',"\n",str_replace(array('\\\\\\','&lt;','&gt;','\\&quot;','&amp;','�'),array('','<','>','"','&',''),$comment))).'"'."<br/><br/>";
...
function get_mostviewed_articles() {
    global $cid;
    $sql = "SELECT DISTINCT article,COUNT(article) AS c FROM (SELECT article, timestamp FROM article_visit AS av INNER JOIN article AS a ON (av.article=a.id) WHERE a.published > FROM_UNIXTIME(UNIX_TIMESTAMP() - 1814400) ORDER BY timestamp DESC LIMIT 500) AS t GROUP BY article ORDER BY c DESC LIMIT 5";
    $rsc = mysql_query($sql,$cid);
    while ($row = mysql_fetch_array($rsc))
...
function clean_content2($text) {
    $result = strip_tags($text, '<p><a><div><b><i><br><blockquote><object><param><embed><li><ul><ol><strong><img><h1><h2><h3><h4><h5><h6><em><iframe><strike><sub><sup><table><tr><td>'); // Gets rid of html tags except <p><a><div>
